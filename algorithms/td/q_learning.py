...
class QLearning(TDAlgoBase):
    ...
    def __init__(self, n_max_iterations: int, tolerance: float,
        ...
        super(QLearning, self).__init__(n_max_iterations=n_max_iterations, tolerance=tolerance,
    ...
    def step(self, **options) -> None:
        ...
        if (self.current_itr_index % self.plot_frequency == 0):
            avg = np.mean(self.tmp_scores)
            print(">>> Train average score {0}".format(avg))
            self.update_avg_scores(avg)

        self._policy.actions_after_episode(self.current_itr_index)
