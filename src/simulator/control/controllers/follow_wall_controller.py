...
class FollowWallController:
    def __init__(self, supervisor):
        ...
        self.l_wall_surface = [
            [1.0, 0.0],
            [1.0, 0.0],
        ]  # the followed surface, in robot space
    ...
    def calculate_fw_heading_vector(self, follow_direction):
        ...
        if follow_direction == FWDIR_LEFT:
            ...
            sensor_detections = self.supervisor.proximity_sensor_positive_detections()[
                7:3:-1
            ]
        ...
        if True not in sensor_detections:
            ...
            sensor_distances, indices = zip(
                *sorted(
                    zip(  # this method ensures two different sensors are always used
        ...
        l_fw_heading_vector = linalg.add(
            l_parallel_component, l_perpendicular_component
        )
