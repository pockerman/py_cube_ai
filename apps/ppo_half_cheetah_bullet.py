"""
Train PPO agent on the HalfCheetahBulletEnv-v0
"""
...
from networks.model_actor_critic import ModelActor, ModelCritic
from agents.a2c_agent import A2CAgent
from utils.experience import ExperienceSource
...
if __name__ == "__main__":
    ...
    # actor model
    net_act = ModelActor(encoder=nn.Sequential(nn.Linear(env.observation_space.shape[0], HID_SIZE),
                         nn.Tanh(), nn.Linear(HID_SIZE, HID_SIZE), nn.Tanh(),
                         nn.Linear(HID_SIZE, env.action_space.shape[0]), nn.Tanh(),),
                         action_size=env.action_space.shape[0]).to(device)

    net_crt = ModelCritic(encoder=nn.Sequential(nn.Linear(env.observation_space.shape[0], HID_SIZE),
                                                nn.ReLU(), nn.Linear(HID_SIZE, HID_SIZE),
                                                nn.ReLU(), nn.Linear(HID_SIZE, 1), )).to(device)
    ...
    agent = A2CAgent(net=net_act, device=device)
